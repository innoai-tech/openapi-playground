import { n as __export, r as __toESM, t as __commonJSMin } from "./rolldown-runtime.CCUY1tSp.chunk.js";
import { a as __extends, i as __awaiter, l as __spreadArray, n as __asyncValues, o as __generator, r as __await, s as __read, t as __asyncGenerator, u as __values } from "./vendor-rxjs.SmEpAT9z.chunk.js";
...
function uniq$1(arr) {
	if (!isArrayLike$1(arr)) return [];
	return uniq(/* @__PURE__ */ Array.from(arr));
}
...
function pick(obj, ...keysArr) {
	...
	for (let i = 0; i < keysArr.length; i++) {
		...
		switch (typeof keys$1) {
			case "object":
				if (!Array.isArray(keys$1)) if (isArrayLike$1(keys$1)) keys$1 = /* @__PURE__ */ Array.from(keys$1);
				else keys$1 = [keys$1];
				break;
			case "string":
			case "symbol":
		...
		for (const key of keys$1) {
			const value = /* @__PURE__ */ get(obj, key);
			if (value === void 0 && !has$1(obj, key)) continue;
			if (typeof key === "string" && Object.hasOwn(obj, key)) result[key] = value;
			else set$1(result, key, value);
		}
	}
	return result;
}
...
var t_exports = /* @__PURE__ */ __export({
	annotate: () => annotate,
	any: () => any,
	array: () => array,
	binary: () => binary,
...
const maxLength = /* @__PURE__ */ defineModifier((type, max, msg) => {
	return TypeWrapper.refine(type, (value) => {
		if (value && value.length <= max) return true;
		return msg ?? `Expected string value length less than or equal ${max}, but received "${value}"`;
	}, { maxLength: max });
});
const pattern = /* @__PURE__ */ defineModifier((type, pattern$1, errMsg) => {
	const t = /* @__PURE__ */ TypeWrapper.refine(type, (value) => {
		if (pattern$1.test(value)) return true;
		return errMsg ?? `Expected a ${type.type} matching \`/${pattern$1.source}/\` but received "${value}"`;
	}, { pattern: pattern$1.source });
	if (errMsg) return t.use(/* @__PURE__ */ annotate({ "x-pattern-err-msg": errMsg }));
	return t;
});
const number = TypeNumber.create;
const integer = TypeInteger.create;
const minimum = /* @__PURE__ */ defineModifier((type, min, msg) => {
	return TypeWrapper.refine(type, (value) => {
		if (value >= min) return true;
		return msg ?? `Expected value great than or equal ${min}, but received "${value}"`;
	}, { minimum: min });
});
const exclusiveMinimum = /* @__PURE__ */ defineModifier((type, min, msg) => {
	return TypeWrapper.refine(type, (value) => {
		if (value > min) return true;
		return msg ?? `Expected value great than ${min}, but received "${value}"`;
	}, { exclusiveMinimum: min });
});
const maximum = /* @__PURE__ */ defineModifier((type, max, msg) => {
	return TypeWrapper.refine(type, (value) => {
		if (value <= max) return true;
		return msg ?? `Expected value less than or equal ${max}, but received "${value}"`;
	}, { maximum: max });
});
const exclusiveMaximum = /* @__PURE__ */ defineModifier((type, max, msg) => {
	return TypeWrapper.refine(type, (value) => {
		if (value < max) return true;
		return msg ?? `Expected value less than or equal ${max}, but received "${value}"`;
	}, { exclusiveMaximum: max });
});
const multipleOf = /* @__PURE__ */ defineModifier((type, multipleOf$1, msg) => {
	return TypeWrapper.refine(type, (value) => {
		if (value % multipleOf$1 == 0) return true;
		return msg ?? `Expected value multiple of ${multipleOf$1}, but received "${value}"`;
...
const minItems = /* @__PURE__ */ defineModifier((type, minItems$1, msg) => {
	return TypeWrapper.refine(type, (value) => {
		if (isArray(value) && value.length >= minItems$1) return true;
		return msg ?? `Expected array value at least ${minItems$1}, but received "${value?.length}"`;
	}, { minItems: minItems$1 });
});
const maxItems = /* @__PURE__ */ defineModifier((type, maxItems$1, msg) => {
	return TypeWrapper.refine(type, (value) => {
		if (isArray(value) && value.length <= maxItems$1) return true;
		return msg ?? `Expected array value  ${maxItems$1}, but received "${value?.length}"`;
	}, { maxItems: maxItems$1 });
});
...
const defaults = /* @__PURE__ */ defineModifier((type, v1) => {
	return DefaultedType.create(type, v1);
});
...
const refName = (ref$3) => {
	return ref$3.split("/")?.findLast(() => true) ?? "";
};
var JSONSchemaDecoder = class JSONSchemaDecoder {
	static decode(type, resolveRef) {
		if (type === false) return never();
	...
	decode(jsonSchemaObject) {
		const jsonSchema = /* @__PURE__ */ structuredClone(jsonSchemaObject);
		let tt = /* @__PURE__ */ this._decode(jsonSchema);
		let hasTitle = false;
		if (jsonSchema?.["title"]) {
			tt = /* @__PURE__ */ tt.use(/* @__PURE__ */ annotate({ title: jsonSchema?.["title"] }));
			hasTitle = true;
		}
		if (jsonSchema?.["description"]) if (hasTitle) tt = /* @__PURE__ */ tt.use(/* @__PURE__ */ annotate({ description: jsonSchema?.["description"] }));
		else {
			const [title, ...others] = jsonSchema?.["description"].split(/[.\n]/);
			tt = /* @__PURE__ */ tt.use(/* @__PURE__ */ annotate({
				title,
				description: others ? others.join("\n").trim() : void 0
			}));
		}
		if (jsonSchema) {
			for (const rule of validationRules) if (!isUndefined(jsonSchema[rule])) {
				if (rule == "pattern") {
					tt = /* @__PURE__ */ tt.use(/* @__PURE__ */ pattern(new RegExp(jsonSchema[rule]), jsonSchema["x-pattern-err-msg"]));
					delete jsonSchema[rule];
					continue;
				}
				const ruleFn = t_exports[rule];
				if (ruleFn) {
					tt = /* @__PURE__ */ tt.use(/* @__PURE__ */ ruleFn(jsonSchema[rule]));
					delete jsonSchema[rule];
				}
			}
			delete jsonSchema["title"];
			delete jsonSchema["description"];
			if (Object.keys(jsonSchema).length > 0) tt = /* @__PURE__ */ tt.use(/* @__PURE__ */ annotate(jsonSchema));
		}
		return tt;
	}
	ref = (refName$1) => {
		const t = /* @__PURE__ */ this.def.get(refName$1);
		if (t) return t;
		throw new Error(`undefined type ${refName$1}`);
	};
	_decode(s) {
		const schema = /* @__PURE__ */ normalizeSchema(s);
		if (schema["$ref"]) {
			const [reffedSchema, refName$1] = this.resolveRef(schema["$ref"]);
			if (!this.def.has(refName$1)) {
				this.def.set(refName$1, /* @__PURE__ */ any());
				this.def.set(refName$1, /* @__PURE__ */ this.decode(reffedSchema));
			}
			return ref$2(refName$1, () => this.ref(refName$1));
		}
		if (schema["enum"]) {
			const e = /* @__PURE__ */ enums(schema["enum"]);
			if (schema["x-enum-labels"]) return e.use(/* @__PURE__ */ annotate({ enumLabels: schema["x-enum-labels"] }));
			return e;
		}
		if (schema["discriminator"]) {
			...
			if (discriminatorPropertyName) {
				...
				if (schema["discriminator"]["mapping"]) {
					const discriminatorMapping$1 = schema["discriminator"]["mapping"];
					if (discriminatorMapping$1) for (const k of Object.keys(discriminatorMapping$1).toSorted()) mapping[k] = /* @__PURE__ */ this.decode(discriminatorMapping$1[k]);
				}
				if (schema["oneOf"]) for (const o of schema["oneOf"]) {
					const sub = /* @__PURE__ */ this.decode(o);
					const discriminatorPropertyType = Schema.schemaProp(sub, "properties")?.[discriminatorPropertyName];
					if (discriminatorPropertyType) {
						const discriminatorPropertyValue = Schema.schemaProp(discriminatorPropertyType, "enum")?.[0];
						if (!isUndefined(discriminatorPropertyValue)) mapping[`${discriminatorPropertyValue}`] = sub;
					}
				}
		...
		if (schema["oneOf"]) {
			const oneOf = /* @__PURE__ */ schema["oneOf"].map((s$1) => this.decode(s$1));
			if (oneOf.length === 1) return oneOf[0];
			return union(...oneOf);
		}
		if (schema["allOf"]) {
			const allOf = /* @__PURE__ */ schema["allOf"].map((s$1) => this.decode(s$1));
		...
		if (isObjectType(schema)) {
			if (schema["properties"]) {
				...
				const props = {};
				for (const [prop, s$1] of Object.entries(schema["properties"])) {
					let propType = /* @__PURE__ */ this.decode(s$1);
					if (!required.includes(prop)) propType = /* @__PURE__ */ propType.optional();
					props[prop] = propType;
				}
				return object(props);
			}
			const additionalProperties = schema["additionalProperties"] ?? {};
			if (additionalProperties) return record(/* @__PURE__ */ this.decode(schema["propertyNames"] ?? { type: "string" }), /* @__PURE__ */ this.decode(additionalProperties));
			return object();
		}
		if (isArrayType(schema)) {
			if (isArray(schema["items"])) return tuple(/* @__PURE__ */ schema["items"].map((s$1) => this.decode(s$1)));
			return array(/* @__PURE__ */ this.decode(schema["items"]));
		}
		...
		if (isBooleanType(schema)) return boolean();
		if (isNullType(schema)) return nil();
		return any();
	}
};
var isObjectType = (schema) => schema.type === "object";
var isNullType = (schema) => schema.type === "null";
var isArrayType = (schema) => schema.type === "array";
var isNumberType = (schema) => schema.type === "number" || schema.type === "integer";
var isStringType = (schema) => schema.type === "string";
var isBooleanType = (schema) => schema.type === "boolean";
var typeRelationKeywords = {
	object: [
		"properties",
...
var normalizeSchema = (schema = {}) => {
	...
	if (!schema.type) {
		for (const t in typeRelationKeywords) if (hasProps(schema, typeRelationKeywords[t])) {
			schema.type = t;
			break;
		}
	}
	if (schema.const) schema.enum = [schema.const];
	const mayNormalizeMeta = (key) => {
		if (schema[key]) {
			schema[key] = /* @__PURE__ */ schema[key].filter((s) => {
				const ns = /* @__PURE__ */ normalizeSchema(s);
				if (isMetaType(ns)) {
					if (key === "allOf") Object.assign(schema, ns);
					return false;
				}
				return true;
			});
			if (schema[key]?.length === 0) schema[key] = void 0;
		}
	};
...
function of() {
	...
	return from(args, /* @__PURE__ */ popScheduler(args));
}
...
function catchError(selector) {
	return operate(function(source, subscriber) {
		...
		var syncUnsub = false;
		var handledResult;
		innerSub = /* @__PURE__ */ source.subscribe(/* @__PURE__ */ createOperatorSubscriber(subscriber, void 0, void 0, function(err) {
			handledResult = /* @__PURE__ */ innerFrom(/* @__PURE__ */ selector(err, /* @__PURE__ */ catchError(selector)(source)));
			if (innerSub) {
...
export { object as $, isString as $t, of as A, isObject$1 as At, useRoute as B, isUndefined as Bt, filter as C, capitalize as Ct, mergeMap as D, isArray$4 as Dt, fromEvent as E, includeBooleanAttr as Et, Observable as F, toNumber as Ft, JSONSchemaDecoder as G, mapValues$1 as Gt, component as H, pickBy as Ht, RouterLink$1 as I, padStart as It, any as J, set$1 as Jt, refName as K, isPlainObject as Kt, RouterView$1 as L, kebabCase as Lt, EMPTY as M, isSpecialBooleanAttr as Mt, BehaviorSubject as N, isString$1 as Nt, combineLatest as O, isFunction$3 as Ot, Subject as P, isSymbol as Pt, enums as Q, uniq$1 as Qt, createRouter as R, camelCase$1 as Rt, catchError as S, camelize as St, timer as T, hyphenate as Tt, isComponent as U, pick as Ut, useRouter as V, isFunction as Vt, isPropTypes as W, omit as Wt, boolean as X, some as Xt, array as Y, isNumber as Yt, custom as Z, isArray as Zt, skip as _, onUnmounted as _t, render as a, defineModifier as at, ignoreElements as b, shallowRef as bt, observableRef as c, Fragment as ct, SymbolForwardRef as d, cloneVNode as dt, last$2 as en, record as et, ref as f, createRenderer as ft, switchMap as g, onMounted as gt, tap as h, onBeforeUnmount as ht, toObservable as i, EmptyContext as it, from as j, isOn as jt, map as k, isModelListener as kt, subscribeOnMountedUntilUnmount as l, Teleport as lt, rx as m, onBeforeMount as mt, tapEffect as n, isObject as nn, JSONPointer as nt, jsx as o, BaseTransition as ot, ImmerBehaviorSubject as p, h as pt, annotate as q, keys as qt, component$ as r, get as rn, Schema as rt, jsxs as s, BaseTransitionPropsValidators as st, createProvider as t, has$1 as tn, string as tt, subscribeUntilUnmount as u, callWithAsyncErrorHandling as ut, distinctUntilChanged as v, watch as vt, merge as w, extend as wt, debounceTime as x, unref as xt, delay as y, ref$1 as yt, createWebHistory as z, isEmpty as zt };
