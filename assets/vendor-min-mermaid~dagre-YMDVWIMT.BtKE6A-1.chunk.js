...
import { r as rr, V as Vt, F as Ft, D as Dt, J as Jt } from './vendor-min-mermaid~chunk-CF2LXAHU.P5w0t2CS.chunk.js';
import { n as ng, w as wo, M, i as ig, a as ag, l as lg, S as So } from './vendor-min-mermaid~chunk-2BQHA3SY.DoMtHC3O.chunk.js';
import { n } from './vendor-min-mermaid~chunk-LM6QDVU5.Cd9M_jtg.chunk.js';
import { m as m$1 } from './vendor-min-mermaid~chunk-GTKDMUJJ.CzbCS-C_.chunk.js';
import { t as tt, H as Hl } from './vendor-min-mermaid~chunk-RAMAZW42.vc82q0BN.chunk.js';
...
Parent cluster`, s.height), t.setNode(s.id, w), t.parent(f) || (tt.trace("Setting parent", f, s.id), t.setParent(f, s.id, w));
        ...
        if (tt.info("(Insert) Node XXX" + f + ": " + JSON.stringify(/*#__PURE__*/ t.node(f))), r?.clusterNode) {
            ...
            M(r, D), r.diff = N.diff || 0, tt.info("New compound node after recursive render XAX", f, "width", r.width, "height", r.height), ig(D, r);
            ...
            node: r
        })) : (tt.trace("Node - the non recursive path XAX", f, b, /*#__PURE__*/ t.node(f), l), await ag(b, /*#__PURE__*/ t.node(f), {
            config: o,
            dir: l
        }));
    })), await m$1(async ()=>{
    ...
    return await Promise.all(/*#__PURE__*/ te(t).map(async function(f) {
        let r = /*#__PURE__*/ t.node(f);
        if (tt.info("Position XBX => " + f + ": (" + r.x, "," + r.y, ") width: ", r.width, " height: ", r.height), r?.clusterNode) r.y += S, tt.info("A tainted cluster node XBX1", f, r.id, r.width, r.height, r.x, r.y, /*#__PURE__*/ t.parent(f)), a.get(r.id).node = r, lg(r);
            ...
            tt.debug("OffsetY", N, "labelHeight", E, "halfPadding", w), await So(d, r), a.get(r.id).node = r;
        } else {
            let w = /*#__PURE__*/ t.node(r.parentId);
            r.y += S / 2, tt.info("A regular node XBX1 - using the padding", r.id, "parent", r.parentId, r.width, r.height, r.x, r.y, "offsetY", r.offsetY, "parent", w, w?.offsetY, r), lg(r);
        }
    ...
    rr(c, e.markers, e.type, e.diagramId), ng(), Vt(), wo(), Z(), e.nodes.forEach((o)=>{
        n.setNode(o.id, {
            ...o
