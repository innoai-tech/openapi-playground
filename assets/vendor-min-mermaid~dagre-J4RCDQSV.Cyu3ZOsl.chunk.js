import { f as fn } from './vendor-min-mermaid~chunk-FASC7IG4.BRsVODVZ.chunk.js';
import { m } from './vendor-min-mermaid~chunk-ZN7TASNU.DoJPj5tk.chunk.js';
import { t as tr, G as Gt, V as Vt, J as Jt, F as Ft } from './vendor-min-mermaid~chunk-QTJCGBHB.F-krgANJ.chunk.js';
import { o as og, x as xo, M, r as rg, s as sg, a as ag, y as yo } from './vendor-min-mermaid~chunk-USR3SDWQ.BTTdHODe.chunk.js';
...
import { t as tt, H as Hl } from './vendor-min-mermaid~chunk-ZKYS2E5M.BBMfbsl4.chunk.js';
import { s as sn, _ as _f, J as Jr } from './vendor-min-mermaid~chunk-5ZJXQJOJ.DbrINuy3.chunk.js';
...
m$1(se, "writeEdges");
var a = new Map, v = new Map, W = new Map, Z = m$1(()=>{
    v.clear(), W.clear(), a.clear();
}, "clear"), J = m$1((e, t)=>{
    let n = v.get(t) || [];
    return tt.trace("In isDescendant", t, " ", e, " = ", n.includes(e)), n.includes(e);
    ...
    e !== c && s.push(e), tt.warn("Copying (nodes) clusterId", e, "nodes", s), s.forEach((o)=>{
        if (t.children(o).length > 0) $(o, t, n, c);
            ...
            let l = t.node(o);
            tt.info("cp ", o, " to ", c, " with parent ", e), n.setNode(o, l), c !== t.parent(o) && (tt.warn("Setting parent", o, t.parent(o)), n.setParent(o, t.parent(o))), e !== c && o !== e ? (tt.debug("Setting parent", o, e), n.setParent(o, e)) : (tt.info("In copy ", e, "root", c, "data", t.node(e), c), tt.debug("Not Setting parent for node=", o, "cluster!==rootId", e !== c, "node!==clusterId", o !== e));
            let u = t.edges(o);
            tt.debug("Copying Edges", u), u.forEach((d)=>{
                tt.info("Edge", d);
                let m = t.edge(d.v, d.w, d.name);
                tt.info("Edge data", m, c);
                try {
                    ce(d, c) ? (tt.info("Copying as ", d.v, d.w, m, d.name), n.setEdge(d.v, d.w, m, d.name), tt.info("newGraph edges ", n.edges(), n.edge(n.edges()[0]))) : tt.info("Skipping copy of edge ", d.v, "-->", d.w, " rootId: ", c, " clusterId:", e);
                } catch (p) {
                    tt.error(p);
                }
            });
...
var ae = m$1((e, t, n)=>{
    let c = e.edges().filter((d)=>d.v === t || d.w === t), s = e.edges().filter((d)=>d.v === n || d.w === n), o = c.map((d)=>({
            v: d.v === t ? n : d.v,
            w: d.w === t ? t : d.w
        })), l = s.map((d)=>({
            v: d.v,
    ...
    e.nodes().forEach(function(n) {
        e.children(n).length > 0 && (tt.warn("Cluster identified", n, " Replacement id in edges: ", x(n, e, n)), v.set(n, L(n, e)), a.set(n, {
            id: x(n, e, n),
        ...
        c.length > 0 ? (tt.debug("Cluster identified", n, v), s.forEach((o)=>{
            ...
            l ^ u && (tt.warn("Edge: ", o, " leaves cluster ", n), tt.warn("Descendants of XXX ", n, ": ", v.get(n)), a.get(n).externalConnections = true);
    ...
    e.edges().forEach(function(n) {
        let c = e.edge(n);
        tt.warn("Edge " + n.v + " -> " + n.w + ": " + JSON.stringify(n)), tt.warn("Edge " + n.v + " -> " + n.w + ": " + JSON.stringify(e.edge(n)));
        let s = n.v, o = n.w;
        if (tt.warn("Fix XXX", a, "ids:", n.v, n.w, "Translating: ", a.get(n.v), " --- ", a.get(n.w)), a.get(n.v) || a.get(n.w)) {
            if (tt.warn("Fixing and trying - removing XXX", n.v, n.w, n.name), s = Q(n.v), o = Q(n.w), e.removeEdge(n.v, n.w, n.name), s !== n.v) {
                let l = e.parent(s);
    ...
    tt.debug("Nodes = ", n, t);
    for (let s of n)if (tt.debug("Extracting node", s, a, a.has(s) && !a.get(s).externalConnections, !e.parent(s), e.node(s), e.children("D"), " Depth ", t), !a.has(s)) tt.debug("Not a cluster", s, t);
    ...
    n = e.nodes(), tt.warn("New list of nodes", n);
    for (let s of n){
        let o = e.node(s);
...
var ie = m$1(async (e, t, n$1, c, s, o)=>{
    ...
    let d = u.insert("g").attr("class", "clusters"), m = u.insert("g").attr("class", "edgePaths"), p = u.insert("g").attr("class", "edgeLabels"), b = u.insert("g").attr("class", "nodes");
...
Parent cluster`, s.height), t.setNode(s.id, w), t.parent(f) || (tt.trace("Setting parent", f, s.id), t.setParent(f, s.id, w));
        ...
        if (tt.info("(Insert) Node XXX" + f + ": " + JSON.stringify(t.node(f))), r?.clusterNode) {
    ...
    return await Promise.all(te(t).map(async function(f) {
        let r = t.node(f);
        if (tt.info("Position XBX => " + f + ": (" + r.x, "," + r.y, ") width: ", r.width, " height: ", r.height), r?.clusterNode) r.y += S, tt.info("A tainted cluster node XBX1", f, r.id, r.width, r.height, r.x, r.y, t.parent(f)), a.get(r.id).node = r, ag(r);
    ...
    tr(c, e.markers, e.type, e.diagramId), og(), Gt(), xo(), Z(), e.nodes.forEach((o)=>{
        ...
        if (o.start === o.end) {
            let l = o.start, u = l + "---" + l + "---1", d = l + "---" + l + "---2", m = n.node(l);
            n.setNode(u, {
                domId: u,
                id: u,
                parentId: m.parentId,
    ...
    }), tt.warn("Graph at first:", JSON.stringify(h(n))), I(n), tt.warn("Graph after XAX:", JSON.stringify(h(n)));
    let s = Hl();
