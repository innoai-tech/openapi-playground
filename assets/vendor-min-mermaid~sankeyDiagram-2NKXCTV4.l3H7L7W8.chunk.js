import { M as Mf, L as Lf, B as Bf, F as Ff, v as vf, w as wf, H as Hl, Z as zC, i as ih, aI as Vr, P as Pl, S as Sf, g as gC, aK as Yp } from './vendor-min-mermaid~chunk-ZKYS2E5M.BBMfbsl4.chunk.js';
...
var ae = {
    ...
    c === "sandbox" && (x = ih("#i" + r));
    let o = c === "sandbox" ? ih(x.nodes()[0].contentDocument.body) : ih("body"), a = c === "sandbox" ? o.select(`[id="${r}"]`) : ih(`[id="${r}"]`), l = S?.width ?? m.width, k = S?.height ?? m.width, _ = S?.useMaxWidth ?? m.useMaxWidth, p = S?.nodeAlignment ?? m.nodeAlignment, v = S?.prefix ?? m.prefix, C = S?.suffix ?? m.suffix, E = S?.showValues ?? m.showValues, M = u.db.getGraph(), D = ae[p];
