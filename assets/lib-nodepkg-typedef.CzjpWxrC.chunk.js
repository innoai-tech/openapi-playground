...
class TypeRef extends TypeWrapper {
  static create = defineType(
    (name, ref) => {
      return new TypeRef({
        $ref: name,
...
class TypeNull extends TypeUnknown {
...
class TypeNumber extends TypeUnknown {
  static create = defineType(() => {
...
class TypeBinary extends TypeUnknown {
  static create = defineType(() => {
...
class TypeTuple extends TypeUnknown {
...
class TypeIntersection extends TypeUnknown {
...
const minLength = defineModifier(
  (type, min, msg) => {
...
const maxLength = defineModifier(
  (type, max, msg) => {
    return TypeWrapper.refine(
      type,
      (value) => {
...
const pattern = defineModifier(
  (type, pattern2, errMsg) => {
...
const exclusiveMinimum = defineModifier(
  (type, min, msg) => {
    return TypeWrapper.refine(
      type,
      (value) => {
...
const exclusiveMaximum = defineModifier(
  (type, max, msg) => {
    return TypeWrapper.refine(
      type,
      (value) => {
...
const multipleOf = defineModifier(
  (type, multipleOf2, msg) => {
    return TypeWrapper.refine(
      type,
      (value) => {
  ...
  (type, minItems2, msg) => {
    return TypeWrapper.refine(
      ...
      (value) => {
        ...
        return msg ?? `Expected array value at least ${minItems2}, but received "${value?.length}"`;
      },
      {
        minItems: minItems2
      }
  ...
  (type, maxItems2, msg) => {
    return TypeWrapper.refine(
      ...
      (value) => {
        ...
        return msg ?? `Expected array value  ${maxItems2}, but received "${value?.length}"`;
      },
      {
        maxItems: maxItems2
      }
...
class JSONSchemaDecoder {
  ...
  decode(jsonSchemaObject) {
    ...
    let hasTitle = false;
    if (jsonSchema?.["title"]) {
      tt = tt.use(annotate({ title: jsonSchema?.["title"] }));
      hasTitle = true;
    }
  ...
  _decode(s) {
    ...
    if (isArrayType(schema)) {
      if (isArray(schema["items"])) {
        return tuple(
          schema["items"].map((s2) => this.decode(s2))
        );
      }
...
const scriptRel = 'modulepreload';const assetsURL = function(dep) { return "/__APP_BASE_HREF__/"+dep };const seen = {};const __vitePreload = function preload(baseModule, deps, importerUrl) {
...
export { EmptyContext as E, JSONPointer as J, Schema as S, __vitePreload as _, array as a, any as b, annotate as c, defineModifier as d, custom as e, JSONSchemaDecoder as f, getDefaultExportFromCjs as g, refName as h, isType as i, enums as j, boolean as k, object as o, produce as p, record as r, string as s };
