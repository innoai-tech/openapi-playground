import { Q as Qf, J as Jf, Z as Zf, K as Kf, X as Xf, j as jf, h as hh, a2 as p_, k as kh, T as Zr, a as ah, V as Vf, e as XC, aO as kg } from './vendor-min-mermaid~chunk-63ZE7VZ5.CfadwiGU.chunk.js';
...
var tt = [], et = [], Z = new Map, Ht = /*#__PURE__*/ m$1(()=>{
    ...
    t = /*#__PURE__*/ XC.sanitizeText(t, /*#__PURE__*/ hh());
    ...
    nodesMap: Z,
    getConfig: /*#__PURE__*/ m$1(()=>hh().sankey, "getConfig"),
    getNodes: Ut,
    getLinks: Xt,
    getGraph: Gt,
    addLink: Yt,
...
var ae = {
    ...
    let { securityLevel: c, sankey: S } = hh(), m = p_.sankey, x;
    c === "sandbox" && (x = /*#__PURE__*/ kh("#i" + r));
    let o = c === "sandbox" ? kh(x.nodes()[0].contentDocument.body) : kh("body"), a = c === "sandbox" ? o.select(`[id="${r}"]`) : kh(`[id="${r}"]`), l = S?.width ?? m.width, k = S?.height ?? m.width, _ = S?.useMaxWidth ?? m.useMaxWidth, p = S?.nodeAlignment ?? m.nodeAlignment, v = S?.prefix ?? m.prefix, C = S?.suffix ?? m.suffix, E = S?.showValues ?? m.showValues, M = /*#__PURE__*/ u.db.getGraph(), D = ae[p];
...
${v}${Math.round(T * 100) / 100}${C}` : y;
    ...
    N.append("path").attr("d", /*#__PURE__*/ it()).attr("stroke", O).attr("stroke-width", (y)=>Math.max(1, y.width)), ah(void 0, a, 0, _);
}, "draw"), Vt = {
    draw: le
};
