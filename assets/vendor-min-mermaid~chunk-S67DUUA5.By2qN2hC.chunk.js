...
var t = m$1((n) => Array.isArray(n) ? {
  x: n[0],
  y: n[1]
} : n, "pointTransformer"), d = m$1((n) => ({
  x: m$1(function(a, c, e) {
    ...
    let s = Math.abs(t(a).x - t(e[e.length - 1]).x), m = Math.abs(t(a).y - t(e[e.length - 1]).y), i = Math.abs(t(a).x - t(e[0]).x), x = Math.abs(t(a).y - t(e[0]).y), u = o[n.arrowTypeStart], y = o[n.arrowTypeEnd], g = 1;
    if (s < y && s > 0 && m < y) {
      let r = y + g - s;
      r *= f === "right" ? -1 : 1, l -= r;
    }
  ...
  y: m$1(function(a, c, e) {
    ...
    let s = Math.abs(t(a).y - t(e[e.length - 1]).y), m = Math.abs(t(a).x - t(e[e.length - 1]).x), i = Math.abs(t(a).y - t(e[0]).y), x = Math.abs(t(a).x - t(e[0]).x), u = o[n.arrowTypeStart], y = o[n.arrowTypeEnd], g = 1;
    if (s < y && s > 0 && m < y) {
      let r = y + g - s;
      r *= f === "up" ? -1 : 1, l -= r;
    }
