import { m as ms, j as je, x as xe } from './vendor-min-mermaid~chunk-D7JRX7UM.bf2NAxZK.chunk.js';
...
import './vendor-min-mermaid~chunk-IO347I67.DJHmq1cW.chunk.js';
import './vendor-min-mermaid~chunk-L736DJ4U.DfHRCDE5.chunk.js';
...
import { H as Hl, i as ih, t as tt$1, T as Tf, g as gC, c as vc, d as Bc } from './vendor-min-mermaid~chunk-ZKYS2E5M.BBMfbsl4.chunk.js';
...
var V = m$1((e)=>e.append("circle").attr("class", "start-state").attr("r", Hl().state.sizeUnit).attr("cx", Hl().state.padding + Hl().state.sizeUnit).attr("cy", Hl().state.padding + Hl().state.sizeUnit), "drawStartState"), D = m$1((e)=>e.append("line").style("stroke", "grey").style("stroke-dasharray", "3").attr("x1", Hl().state.textHeight).attr("class", "divider").attr("x2", Hl().state.textHeight * 2).attr("y1", 0).attr("y2", 0), "drawDivider"), tt = m$1((e, n)=>{
    ...
    let m = e.append("line").attr("x1", Hl().state.padding).attr("y1", Hl().state.padding + h + Hl().state.dividerMargin / 2).attr("y2", Hl().state.padding + h + Hl().state.dividerMargin / 2).attr("class", "descr-divider"), x = f.node().getBBox(), c = Math.max(x.width, r.width);
...
var at = m$1((e, n, s, d)=>{
    ...
    if (Hl().state.arrowMarkerAbsolute && (i = window.location.protocol + "//" + window.location.host + window.location.pathname + window.location.search, i = i.replace(/\(/g, "\\("), i = i.replace(/\)/g, "\\)")), f.attr("marker-end", "url(" + i + "#" + d(xe.relationType.DEPENDENCY) + "End)"), s.title !== void 0) {
        let o = e.append("g").attr("class", "stateLabel"), { x: m, y: x } = Ie.calcLabelPosition(n.points), c = gC.getRows(s.title), p = 0, y = [], w = 0, k = 0;
        for(let a = 0; a <= c.length; a++){
            let u = o.append("text").attr("text-anchor", "middle").text(c[a]).attr("x", m).attr("y", x + p), l = u.node().getBBox();
            w = Math.max(w, l.width), k = Math.min(k, l.x), tt$1.info(l.x, m, x + p), p === 0 && (p = u.node().getBBox().height, tt$1.info("Title height", p, x)), y.push(u);
        }
        let M = p * c.length;
        if (c.length > 1) {
...
var S, G = {}, ot = m$1(function() {}, "setConf"), st = m$1(function(e) {
