...
import { L as Le, S as S$1 } from './vendor-min-mermaid~chunk-YM3XIQPS.DjUuqvt-.chunk.js';
import { m as m$1 } from './vendor-min-mermaid~chunk-GTKDMUJJ.ByXaayWf.chunk.js';
import { F as Ff, B as Bf, w as wf, v as vf, L as Lf, M as Mf, t as tt, H as Hl, al as bc, aI as Vr, T as Tf, S as Sf, aJ as wc, p as pl } from './vendor-min-mermaid~chunk-ZKYS2E5M.BBMfbsl4.chunk.js';
...
` + e);
    let s = n.db, T = Hl(), b = Le(s.getConfig(), T.pie), v = 40, a = 18, d$1 = 4, c = 450, y = c, P = d(i), p = P.append("g");
    p.attr("transform", "translate(" + y / 2 + "," + c / 2 + ")");
    let { themeVariables: r } = T, [$] = S$1(r.pieOuterStrokeWidth);
    $ ??= 2;
    let G = b.textPosition, f = Math.min(y, c) / 2 - v, te = bc().innerRadius(0).outerRadius(f), ie = bc().innerRadius(f * G).outerRadius(f * G);
    ...
    k.forEach((o)=>{
        E += o;
    }), p.selectAll("mySlices").data(h).enter().append("text").text((o)=>(o.data.value / E * 100).toFixed(0) + "%").attr("transform", (o)=>"translate(" + ie.centroid(o) + ")").style("text-anchor", "middle").attr("class", "slice"), p.append("text").text(s.getDiagramTitle()).attr("x", 0).attr("y", -400 / 2).attr("class", "pieTitleText");
    let x = p.selectAll(".legend").data(l.domain()).enter().append("g").attr("class", "legend").attr("transform", (o, g)=>{
    ...
    let oe = Math.max(...x.selectAll("text").nodes().map((o)=>o?.getBoundingClientRect().width ?? 0)), R = y + v + a + d$1 + oe;
