import { $ as $t$1, n as nr, v } from './vendor-min-mermaid~chunk-USR3SDWQ.BTTdHODe.chunk.js';
...
import { Q as Qt } from './vendor-min-mermaid~chunk-HESFG3RP.DM0RUiMH.chunk.js';
import { I as Ie } from './vendor-min-mermaid~chunk-YM3XIQPS.DjUuqvt-.chunk.js';
import { t as tt, H as Hl, m as mf, i as ih, c as vc, d as Bc, f as go, u as ug, h as fg, G as Gc, Y as Yc, j as cg, k as hg, s as sg, l as eg, n as tg, o as ig } from './vendor-min-mermaid~chunk-ZKYS2E5M.BBMfbsl4.chunk.js';
...
var st = m$1((r, t, a, s, l, i)=>{
    ...
    if (n && n.trim() !== "") {
        let c = n.replace(/[^\dA-Za-z]/g, "_"), m = `${f}_${c}`;
...
var W = new Map, x = new Map, Gt = m$1(()=>{
    W.clear(), x.clear();
    ...
    if (a) {
        ...
        n = e.getBoundingClientRect(), p.attr("width", n.width), p.attr("height", n.height);
    }
...
var Ft = m$1((r, t)=>{
    ...
    let a = r.x, s = r.y, l = Math.abs(t.x - a), i = Math.abs(t.y - s), n = r.width / 2, o = r.height / 2;
    return l >= n || i >= o;
  ...
  node        : x:${r.x} y:${r.y} w:${r.width} h:${r.height}`);
    let s = r.x, l = r.y, i = Math.abs(s - a.x), n = r.width / 2, o = a.x < t.x ? n - i : n + i, e = r.height / 2, p = Math.abs(t.y - a.y), f = Math.abs(t.x - a.x);
...
var it = m$1(function(r, t, a) {
    let s = t.x - r.x, l = t.y - r.y, i = Math.sqrt(s * s + l * l), n = a / i;
    return {
        x: t.x - n * s,
        y: t.y - n * l
    };
    ...
    for(let s = 0; s < r.length; s++)if (t.includes(s)) {
        let l = r[s - 1], i = r[s + 1], n = r[s], o = it(l, n, 5), e = it(i, n, 5), p = e.x - o.x, f = e.y - o.y;
        a.push(o);
    ...
    return a;
}, "fixCorners"), Jt = m$1(function(r, t, a, s, l, i, n) {
    ...
    let w, q = lt(y), M = Array.isArray(t.style) ? t.style : [
        t.style
    ], U = M.find((u)=>u?.startsWith("stroke:"));
    if (t.look === "handDrawn") {
        let u = v.svg(r);
        Object.assign([], y);
        let v$1 = u.path(q, {
        ...
        let u = m.join(";"), v = M ? M.reduce((H, ct)=>H + ct + ";", "") : "", _ = "";
        t.animate && (_ = " edge-animation-fast"), t.animation && (_ = " edge-animation-" + t.animation);
        let X = u ? u + ";" + v + ";" : v;
        w = r.append("path").attr("d", q).attr("id", t.id).attr("class", " " + b + (t.classes ? " " + t.classes : "") + (_ ?? "")).attr("style", X), U = X.match(/stroke:([^;]+)/)?.[1];
    }
    let $ = "";
    (Hl().flowchart.arrowMarkerAbsolute || Hl().state.arrowMarkerAbsolute) && ($ = window.location.protocol + "//" + window.location.host + window.location.pathname + window.location.search, $ = $.replace(/\(/g, "\\(").replace(/\)/g, "\\)")), tt.info("arrowTypeStart", t.arrowTypeStart), tt.info("arrowTypeEnd", t.arrowTypeEnd), st(w, t, $, n, s, U);
    let C = {};
