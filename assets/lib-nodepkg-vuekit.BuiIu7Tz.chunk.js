...
import { i as isType, p as produce, e as custom, o as object } from "./lib-nodepkg-typedef.D-PCYzjh.chunk.js";
import { p as pipe, e as from, O as Observable, S as Subject, b as map, t as tap, B as BehaviorSubject, i as isObservable } from "./vendor-rxjs.Bs9iBULs.chunk.js";
import { b as isFunction$2, p as partition, k as kebabCase, i as isPlainObject$1, a as isUndefined$1, d as isArray$2, h as isEmpty } from "./vendor-innoai-tech-lodash.DimVzCL6.chunk.js";
/**
* @vue/shared v3.5.13
...
const def = function(obj, key, value) {
  let writable = arguments.length > 3 && arguments[3] !== void 0 ? arguments[3] : false;
...
const toNumber = (val) => {
  const n1 = isString(val) ? Number(val) : NaN;
  return isNaN(n1) ? val : n1;
};
let _globalThis;
const getGlobalThis = () => {
...
let batchedComputed;
function batch(sub) {
  let isComputed = arguments.length > 1 && arguments[1] !== void 0 ? arguments[1] : false;
  sub.flags |= 8;
  if (isComputed) {
...
function removeSub(link) {
  let soft = arguments.length > 1 && arguments[1] !== void 0 ? arguments[1] : false;
...
function iterator(self1, method, wrapValue) {
  const arr = shallowReadArray(self1);
  const iter = arr[method]();
  if (arr !== self1 && !isShallow(self1)) {
...
function apply(self1, method, fn, thisArg, wrappedRetFn, args) {
  const arr = shallowReadArray(self1);
  const needsWrap = arr !== self1 && !isShallow(self1);
  ...
  if (arr !== self1) {
    if (needsWrap) {
      wrappedFn = function(item, index) {
        return fn.call(this, toReactive(item), index, self1);
      };
    } else if (fn.length > 2) {
      wrappedFn = function(item, index) {
  ...
  if (arr !== self1) {
    if (!isShallow(self1)) {
      wrappedFn = function(acc, item, index) {
        return fn.call(this, acc, toReactive(item), index, self1);
      };
    } else if (fn.length > 3) {
      wrappedFn = function(acc, item, index) {
...
function noTracking(self1, method) {
  let args = arguments.length > 2 && arguments[2] !== void 0 ? arguments[2] : [];
  pauseTracking();
  startBatch();
  const res = toRaw(self1)[method].apply(self1, args);
  endBatch();
  resetTracking();
  return res;
}
...
function computed$1(getterOrOptions, debugOptions) {
  let isSSR = arguments.length > 2 && arguments[2] !== void 0 ? arguments[2] : false;
  let getter;
  let setter;
...
function onWatcherCleanup(cleanupFn) {
  let owner = arguments.length > 2 && arguments[2] !== void 0 ? arguments[2] : activeWatcher;
  if (owner) {
...
function watch$1(source, cb) {
  let options = arguments.length > 2 && arguments[2] !== void 0 ? arguments[2] : EMPTY_OBJ;
...
function traverse(value) {
  let depth = arguments.length > 1 && arguments[1] !== void 0 ? arguments[1] : Infinity, seen = arguments.length > 2 ? arguments[2] : void 0;
...
function handleError(err, instance, type) {
  let throwInDev = arguments.length > 3 && arguments[3] !== void 0 ? arguments[3] : true;
...
function logError(err, type, contextVNode) {
  let throwInProd = arguments.length > 4 && arguments[4] !== void 0 ? arguments[4] : false;
  if (throwInProd) {
    throw err;
  } else {
    console.error(err);
...
function flushPreFlushCbs(instance, seen) {
  let i = arguments.length > 2 && arguments[2] !== void 0 ? arguments[2] : flushIndex + 1;
...
function withCtx(fn) {
  let ctx = arguments.length > 1 && arguments[1] !== void 0 ? arguments[1] : currentRenderingInstance;
  if (!ctx) return fn;
  if (fn._n) {
    return fn;
  }
...
const TeleportImpl = {
  ...
  remove(vnode, parentComponent, parentSuspense, param, doRemove) {
    let { um: unmount, o: { remove: hostRemove } } = param;
...
function moveTeleport(vnode, container, parentAnchor, param) {
  let { o: { insert }, m: move } = param, moveType = arguments.length > 4 && arguments[4] !== void 0 ? arguments[4] : 2;
  if (moveType === 0) {
...
function hydrateTeleport(node, vnode, parentComponent, parentSuspense, slotScopeIds, optimized, param, hydrateChildren) {
  let { o: { nextSibling, parentNode, querySelector, insert, createText } } = param;
...
function getTransitionRawChildren(children) {
  let keepComment = arguments.length > 1 && arguments[1] !== void 0 ? arguments[1] : false, parentKey = arguments.length > 2 ? arguments[2] : void 0;
  let ret = [];
...
function setRef(rawRef, oldRawRef, parentSuspense, vnode) {
  let isUnmount = arguments.length > 4 && arguments[4] !== void 0 ? arguments[4] : false;
  if (isArray$1(rawRef)) {
...
function registerKeepAliveHook(hook, type) {
  let target = arguments.length > 2 && arguments[2] !== void 0 ? arguments[2] : currentInstance;
...
function injectHook(type, hook) {
  let target = arguments.length > 2 && arguments[2] !== void 0 ? arguments[2] : currentInstance, prepend = arguments.length > 3 && arguments[3] !== void 0 ? arguments[3] : false;
  if (target) {
...
const createHook = (lifecycle) => function(hook) {
  let target = arguments.length > 1 && arguments[1] !== void 0 ? arguments[1] : currentInstance;
...
const PublicInstanceProxyHandlers = {
  ...
  has(param, key) {
    let { _: { data, setupState, accessCache, ctx, appContext, propsOptions } } = param;
    let normalizedProps;
...
function mergeOptions$1(to, from2, strats) {
  let asMixin = arguments.length > 3 && arguments[3] !== void 0 ? arguments[3] : false;
...
function createAppAPI(render2, hydrate) {
  return function createApp2(rootComponent) {
    let rootProps = arguments.length > 1 && arguments[1] !== void 0 ? arguments[1] : null;
...
function inject(key, defaultValue) {
  let treatDefaultAsFactory = arguments.length > 2 && arguments[2] !== void 0 ? arguments[2] : false;
...
function initProps(instance, rawProps, isStateful) {
  let isSSR = arguments.length > 3 && arguments[3] !== void 0 ? arguments[3] : false;
  const props = {};
...
function normalizePropsOptions(comp, appContext) {
  let asMixin = arguments.length > 2 && arguments[2] !== void 0 ? arguments[2] : false;
...
function baseCreateRenderer(options, createHydrationFns) {
  ...
  const patch = function(n1, n2, container) {
    let anchor = arguments.length > 3 && arguments[3] !== void 0 ? arguments[3] : null, parentComponent = arguments.length > 4 && arguments[4] !== void 0 ? arguments[4] : null, parentSuspense = arguments.length > 5 && arguments[5] !== void 0 ? arguments[5] : null, namespace = arguments.length > 6 && arguments[6] !== void 0 ? arguments[6] : void 0, slotScopeIds = arguments.length > 7 && arguments[7] !== void 0 ? arguments[7] : null, optimized = arguments.length > 8 && arguments[8] !== void 0 ? arguments[8] : !!n2.dynamicChildren;
    if (n1 === n2) {
      return;
    }
  ...
  const moveStaticNode = (param, container, nextSibling) => {
    let { el, anchor } = param;
    let next;
    while (el && el !== anchor) {
  ...
  const mountChildren = function(children, container, anchor, parentComponent, parentSuspense, namespace, slotScopeIds, optimized) {
    let start = arguments.length > 8 && arguments[8] !== void 0 ? arguments[8] : 0;
  ...
  const patchChildren = function(n1, n2, container, anchor, parentComponent, parentSuspense, namespace, slotScopeIds) {
    let optimized = arguments.length > 8 && arguments[8] !== void 0 ? arguments[8] : false;
  ...
  const move = function(vnode, container, anchor, moveType) {
    let parentSuspense = arguments.length > 4 && arguments[4] !== void 0 ? arguments[4] : null;
  ...
  const unmount = function(vnode, parentComponent, parentSuspense) {
    let doRemove = arguments.length > 3 && arguments[3] !== void 0 ? arguments[3] : false, optimized = arguments.length > 4 && arguments[4] !== void 0 ? arguments[4] : false;
  ...
  const unmountChildren = function(children, parentComponent, parentSuspense) {
    let doRemove = arguments.length > 3 && arguments[3] !== void 0 ? arguments[3] : false, optimized = arguments.length > 4 && arguments[4] !== void 0 ? arguments[4] : false, start = arguments.length > 5 && arguments[5] !== void 0 ? arguments[5] : 0;
    for (let i = start; i < children.length; i++) {
...
function setBlockTracking(value) {
  let inVOnce = arguments.length > 1 && arguments[1] !== void 0 ? arguments[1] : false;
...
const normalizeKey = (param) => {
  let { key } = param;
  return key != null ? key : null;
};
const normalizeRef = (param) => {
  let { ref: ref3, ref_key, ref_for } = param;
...
function cloneVNode(vnode, extraProps) {
  let mergeRef = arguments.length > 2 && arguments[2] !== void 0 ? arguments[2] : false, cloneTransition = arguments.length > 3 && arguments[3] !== void 0 ? arguments[3] : false;
  ...
  const cloned = {
    ...
    ref: extraProps && extraProps.ref ? (
      ...
      mergeRef && ref3 ? isArray$1(ref3) ? ref3.concat(normalizeRef(extraProps)) : [
        ref3,
...
function createTextVNode() {
  let text = arguments.length > 0 && arguments[0] !== void 0 ? arguments[0] : " ", flag = arguments.length > 1 && arguments[1] !== void 0 ? arguments[1] : 0;
...
function invokeVNodeHook(hook, instance, vnode) {
  let prevVNode = arguments.length > 3 && arguments[3] !== void 0 ? arguments[3] : null;
...
function setupComponent(instance) {
  let isSSR = arguments.length > 1 && arguments[1] !== void 0 ? arguments[1] : false, optimized = arguments.length > 2 && arguments[2] !== void 0 ? arguments[2] : false;
...
function getComponentName(Component) {
  let includeInferred = arguments.length > 1 && arguments[1] !== void 0 ? arguments[1] : true;
  return isFunction$1(Component) ? Component.displayName || Component.name : Component.name || includeInferred && Component.__name;
}
function formatComponentName(instance, Component) {
  let isRoot = arguments.length > 2 && arguments[2] !== void 0 ? arguments[2] : false;
...
const Transition = /* @__PURE__ */ decorate$1((props, param) => {
  let { slots } = param;
...
function patchAttr(el, key, value, isSVG, instance) {
  let isBoolean = arguments.length > 5 && arguments[5] !== void 0 ? arguments[5] : isSpecialBooleanAttr(key);
...
function patchEvent(el, rawName, prevValue, nextValue) {
  let instance = arguments.length > 4 && arguments[4] !== void 0 ? arguments[4] : null;
...
const isComponent = (o) => {
  return isPlainObject$1(o) && o[__component] === __component;
};
const isPropTypes = (o) => {
...
function component() {
  ...
  for (const arg of args) {
    ...
    if (isUndefined$1(finalSetup)) {
      finalPropTypes = arg;
    } else {
      finalOptions = arg;
    }
  }
  const [emits, props] = partition(Object.keys(finalPropTypes), (v1) => /^on[A-Z]/.test(v1));
  const emitsAndProps = {
    emits: [
      ...emits.map((v1) => kebabCase(v1.slice("on".length))),
      ...finalOptions["emits"] ?? []
    ],
    props: [
      ...props.filter((p2) => !/^[$]/.test(p2)),
      ...finalOptions["props"] ?? []
    ].reduce((ret, prop) => {
  ...
  return {
    ...
    setup: (props2, ctx) => finalSetup(props2, ctx),
...
const ext = (target, ext2) => {
  return new Proxy(target, {
    get(target2, p2) {
      return ext2[p2] ?? target2[p2];
    }
  });
};
...
function parseURL(parseQuery2, location2) {
  let currentLocation = arguments.length > 2 && arguments[2] !== void 0 ? arguments[2] : "/";
...
function buildState(back, current, forward) {
  let replaced = arguments.length > 3 && arguments[3] !== void 0 ? arguments[3] : false, computeScroll = arguments.length > 4 && arguments[4] !== void 0 ? arguments[4] : false;
  return {
    back,
    current,
    forward,
...
function createWebHistory(base) {
  ...
  function go(delta) {
    let triggerListeners = arguments.length > 1 && arguments[1] !== void 0 ? arguments[1] : true;
...
function stringifyQuery(query) {
  ...
  for (let key in query) {
    ...
    const values = isArray(value) ? value.map((v1) => v1 && encodeQueryValue(v1)) : [
...
function normalizeQuery(query) {
  ...
  for (const key in query) {
    const value = query[key];
    if (value !== void 0) {
      normalizedQuery[key] = isArray(value) ? value.map((v1) => v1 == null ? null : "" + v1) : value == null ? value : "" + value;
    }
  }
...
function guardToPromiseFn(guard, to, from2, record, name) {
  let runWithContext = arguments.length > 5 && arguments[5] !== void 0 ? arguments[5] : (fn) => fn();
...
function extractComponentsGuards(matched, guardType, to, from2) {
  let runWithContext = arguments.length > 4 && arguments[4] !== void 0 ? arguments[4] : (fn) => fn();
  const guards = [];
...
function useLink(props) {
  ...
  const activeRecordIndex = computed(() => {
    ...
    return length > 1 && // if the parent and matched route have the same path, this link is
...
const RouterViewImpl = /* @__PURE__ */ defineComponent({
  ...
  setup(props, param) {
    ...
    watch(() => [
      ...
      let [instance, to, name] = param2, [oldInstance, from2, oldName] = param1;
      if (to) {
    ...
    return () => {
      ...
      const component2 = h(ViewComponent, assign({}, routeProps, attrs, {
        onVnodeUnmounted,
        ref: viewRef
      }));
      return (
...
const RouterView$1 = RouterViewImpl;
function createRouter(options) {
      ...
      for (const record of enteringRecords) {
        if (record.beforeEnter) {
          if (isArray(record.beforeEnter)) {
            for (const beforeEnter of record.beforeEnter) guards.push(guardToPromiseFn(beforeEnter, to, from2));
          } else {
  ...
  function markAsReady(err) {
    if (!ready) {
      ready = !err;
      setupListeners();
      readyHandlers.list().forEach((param) => {
        let [resolve2, reject] = param;
...
const RouterLink = RouterLink$1;
const RouterView = RouterView$1;
function rx(source) {
  for (var _len = arguments.length, operations = new Array(_len > 1 ? _len - 1 : 0), _key = 1; _key < _len; _key++) {
    operations[_key - 1] = arguments[_key];
  }
  return pipe(...operations)(from(source));
}
class ImmerBehaviorSubject extends Observable {
  constructor(seed) {
    super((subscriber) => {
  ...
  next(valueOrUpdater, defaultValue) {
    const v1 = isFunction$2(valueOrUpdater) ? produce(this._value ?? defaultValue, valueOrUpdater) : valueOrUpdater;
    if (!Object.is(v1, this._value)) {
      this._subject$.next(this._value = v1);
    }
  }
}
const SymbolForwardRef = Symbol("forwardRef");
function ref(defaultValue) {
  ...
  return customRef((track2, trigger2) => {
    return {
      ...
      set(value) {
        const newValue = (value == null ? void 0 : value[SymbolForwardRef]) ?? value;
...
const isFragment = (val) => val === Fragment;
const isTagOrInternal = (val) => {
  if (isFragment(val)) {
    return true;
  }
...
const pickPropsWithoutSlots = (rawProps, key) => {
  ...
  return [
    ...
    hasAnySlot ? slots : void 0
  ];
};
const jsxs = (type, rawProps, key) => {
  return jsx(type, rawProps, key);
};
const jsx = (type, rawProps, key) => {
  var _a;
  const [props, slots] = pickPropsWithoutSlots(rawProps, key);
  if (isTagOrInternal(type)) {
    const children = ((_a = slots == null ? void 0 : slots.default) == null ? void 0 : _a.call(slots)) ?? (isFragment(type) ? [] : void 0);
...
function subscribeOnMountedUntilUnmount(observerOrNext) {
  return (ob$) => {
    let sub;
    onMounted(() => {
      sub = ob$.subscribe(observerOrNext);
    });
    onBeforeUnmount(() => sub == null ? void 0 : sub.unsubscribe());
  };
}
function render(renderFunc) {
  return (input$) => {
    return /* @__PURE__ */ jsx(RxSlot, {
      elem$: input$.pipe(map((v1) => () => renderFunc(v1))),
      children: {}
    });
  };
}
const RxSlot = component({
  ...
  rx(props.elem$, tap((renderFunc) => {
    r.value = renderFunc;
  }), subscribeUntilUnmount());
  return () => {
    var _a;
    return (_a = r.value) == null ? void 0 : _a.call(r);
  };
}, {
  name: "RxSlot"
});
...
const toObservables = (obj) => {
  const refs = {};
  for (const k in obj) {
    if (isFunction$2(obj[k]) || isObservable(obj[k])) {
      refs[k] = obj[k];
      continue;
    }
...
function component$() {
  ...
  return component(finalPropTypes, (props, ctx) => {
    ...
    const renderFuncOrVNode = finalSetup(p2, c);
    if (isFunction$2(renderFuncOrVNode)) {
...
const observableRef = (value) => {
  ...
  const ref2 = customRef((track2, trigger2) => {
    return {
      ...
      set(value2) {
        const newValue = (value2 == null ? void 0 : value2[SymbolForwardRef]) ?? value2;
    ...
    get(_, p2) {
      if (p2 === "next") {
        return (valueOrUpdater) => {
          ref2.value = isFunction$2(valueOrUpdater) ? produce(store$.value, valueOrUpdater) : valueOrUpdater;
        };
      }
      if (p2 === "value") {
        return ref2.value;
      }
      return store$[p2] ?? ref2[p2];
    },
    set(value$, p2, newValue) {
      if (p2 === "value") {
        ref2.value = newValue;
        return true;
      }
...
function createProvider() {
  ...
  for (const arg of args) {
    ...
    if (isUndefined$1(finalCreate)) {
      finalPropTypes = arg;
    } else {
      finalOptions = arg;
    }
  }
  const key = Symbol((finalOptions == null ? void 0 : finalOptions["name"]) ?? "");
  if (isEmpty(finalPropTypes) && isEmpty(finalOptions["props"])) {
    let _default2;
    const getDefaults = () => {
      return _default2 ?? (_default2 = finalCreate({}));
    };
    const Provider2 = component({
      ...
      ...finalOptions,
      name: `Provide(${(finalOptions == null ? void 0 : finalOptions["name"]) ?? ""})`
    });
    return ext(Provider2, {
      use: () => {
        return inject(key, getDefaults, true);
      }
    });
  }
  const propsSchema = object(finalPropTypes);
  ...
  return ext(Provider, {
    use: () => {
      return inject(key, () => {
        return _default ?? (_default = finalCreate(getDefaultProps()));
      }, true);
    }
  });
}
export {
  ...
  useRoute as D,
  Fragment as F,
  ImmerBehaviorSubject as I,
  RouterView as R,
  SymbolForwardRef as S,
  Teleport as T,
  component$ as a,
  cloneVNode as b,
  createProvider as c,
  ...
  toObservable as q,
  rx as r,
  subscribeUntilUnmount as s,
  tapEffect as t,
  unref as u,
  subscribeOnMountedUntilUnmount as v,
  watch as w,
  createRouter as x,
