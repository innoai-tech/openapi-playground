...
import { L as Lt } from './vendor-min-mermaid~chunk-EFRVIJHI.wT3iKf0B.chunk.js';
import { m as m$1 } from './vendor-min-mermaid~chunk-GTKDMUJJ.CzbCS-C_.chunk.js';
import { O as Ot, W as Wo, H as Ho, U as Uo, j as jo, V as Vo, $ as $o, G as Go, a as Ni, N as No } from './vendor-min-mermaid~chunk-KXVH62NG.CUpUCMGK.chunk.js';
import { f as $n, p as pt$1, g as He, P as Pn, U as Ua, i as ia } from './vendor-min-mermaid~chunk-63GW7ZVL.CUmzAGpZ.chunk.js';
var $ = class {
    constructor(){
        this.nodes = [];
        this.levels = new Map;
    ...
    clear() {
        No(), this.nodes = [], this.levels = new Map, this.outerNodes = [], this.classes = new Map, this.root = void 0;
    }
};
function fe(m) {
    if (!m.length) return [];
...
var De = /*#__PURE__*/ m$1((m, a)=>{
    ...
    for (let l of m.TreemapRows ?? []){
        let d = l.item;
        if (!d) continue;
        let g = l.indent ? parseInt(l.indent) : 0, W = /*#__PURE__*/ ve(d), r = d.classSelector ? a.getStylesForClass(d.classSelector) : [], P = r.length > 0 ? r.join(";") : void 0, T = {
            level: g,
            name: W,
            type: d.$type,
            value: d.value,
    ...
    let o = /*#__PURE__*/ fe(n), c$1 = m$1((l, d)=>{
        for (let g of l)a.addNode(g, d), g.children && g.children.length > 0 && c$1(g.children, d + 1);
    }, "addNodesRecursively");
    c$1(o, 0);
    ...
    parse: /*#__PURE__*/ m$1(async (m)=>{
        try {
            ...
            throw pt$1.error("Error parsing treemap:", a), a;
        }
    }, "parse")
};
var we = 10, F = 10, E = 25, Ne = /*#__PURE__*/ m$1((m$2, a, n, o)=>{
    let c = o.db, l = /*#__PURE__*/ c.getConfig(), d$1 = l.padding ?? we, g = /*#__PURE__*/ c.getDiagramTitle(), W = /*#__PURE__*/ c.getRoot(), { themeVariables: r } = Ot();
    if (!W) return;
    let P = g ? 30 : 0, T = /*#__PURE__*/ d(a), X = l.nodeWidth ? l.nodeWidth * F : 960, Y = l.nodeHeight ? l.nodeHeight * F : 500, G = X, q = Y + P;
    ...
    try {
        ...
        pt$1.error("Error creating format function:", e), C = /*#__PURE__*/ $n(",");
    }
    ...
    g && T.append("text").attr("x", G / 2).attr("y", P / 2).attr("class", "treemapTitle").attr("text-anchor", "middle").attr("dominant-baseline", "middle").text(g);
    let U = /*#__PURE__*/ T.append("g").attr("transform", `translate(0, ${P})`).attr("class", "treemapContainer"), ye = /*#__PURE__*/ Pn(W).sum((e)=>e.value ?? 0).sort((e, t)=>(t.value ?? 0) - (e.value ?? 0)), J = /*#__PURE__*/ Ua().size([
        X,
        Y
    ...
    k.append("rect").attr("width", (e)=>e.x1 - e.x0).attr("height", E).attr("class", "treemapSectionHeader").attr("fill", "none").attr("fill-opacity", .6).attr("stroke-width", .6).attr("style", (e)=>e.depth === 0 ? "display: none;" : ""), k.append("clipPath").attr("id", (e, t)=>`clip-section-${a}-${t}`).append("rect").attr("width", (e)=>Math.max(0, e.x1 - e.x0 - 12)).attr("height", E), k.append("rect").attr("width", (e)=>e.x1 - e.x0).attr("height", (e)=>e.y1 - e.y0).attr("class", (e, t)=>`treemapSection section${t}`).attr("fill", (e)=>A(e.data.name)).attr("fill-opacity", .6).attr("stroke", (e)=>he(e.data.name)).attr("stroke-width", 2).attr("stroke-opacity", .4).attr("style", (e)=>{
        ...
        if (e.depth === 0) return "display: none;";
        let t = "dominant-baseline: middle; font-size: 12px; fill:" + B(e.data.name) + "; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;", s = /*#__PURE__*/ m({
            cssCompiledStyles: e.data.cssCompiledStyles
        });
        ...
        t.text(s);
        let f = e.x1 - e.x0, y = 6, S;
        l.showValues !== false && e.value ? S = f - 10 - 30 - 10 - y : S = f - y - 6;
        ...
        if (e.depth === 0) return "display: none;";
        let t = "text-anchor: end; dominant-baseline: middle; font-size: 10px; fill:" + B(e.data.name) + "; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;", s = /*#__PURE__*/ m({
            cssCompiledStyles: e.data.cssCompiledStyles
        });
        return t + s.labelStyles.replace("color:", "fill:");
    });
    ...
    R.append("rect").attr("width", (e)=>e.x1 - e.x0).attr("height", (e)=>e.y1 - e.y0).attr("class", "treemapLeaf").attr("fill", (e)=>e.parent ? A(e.parent.data.name) : A(e.data.name)).attr("style", (e)=>m({
            cssCompiledStyles: e.data.cssCompiledStyles
        ...
        let t = "text-anchor: middle; dominant-baseline: middle; font-size: 38px;fill:" + B(e.data.name) + ";", s = /*#__PURE__*/ m({
            cssCompiledStyles: e.data.cssCompiledStyles
        });
        return t + s.labelStyles.replace("color:", "fill:");
    }).attr("clip-path", (e, t)=>`url(#clip-${a}-${t})`).text((e)=>e.data.name).each(function(e) {
        let t = /*#__PURE__*/ ia(this), s = e.x1 - e.x0, f = e.y1 - e.y0, y = /*#__PURE__*/ t.node(), S = 4, w = s - 2 * S, u = f - 2 * S;
        if (w < 10 || u < 10) {
            t.style("display", "none");
            return;
        }
        let i = /*#__PURE__*/ parseInt(/*#__PURE__*/ t.style("font-size"), 10), D = 8, b = 28, h = .6, x = 6, z = 2;
        for(; y.getComputedTextLength() > w && i > D;)i--, t.style("font-size", `${i}px`);
        ...
        t.style("font-size", `${i}px`), (y.getComputedTextLength() > w || i < D || u < i) && t.style("display", "none");
    }), l.showValues !== false && R.append("text").attr("class", "treemapValue").attr("x", (t)=>(t.x1 - t.x0) / 2).attr("y", function(t) {
        return (t.y1 - t.y0) / 2;
    }).attr("style", (t)=>{
        let s = "text-anchor: middle; dominant-baseline: hanging; font-size: 28px;fill:" + B(t.data.name) + ";", f = /*#__PURE__*/ m({
        ...
        let y = /*#__PURE__*/ ia(f).select(".treemapLabel");
        if (y.empty() || y.style("display") === "none") {
            s.style("display", "none");
            return;
        }
        let S = /*#__PURE__*/ parseFloat(/*#__PURE__*/ y.style("font-size")), w = 28, u = .6, i = 6, D = 2, b = /*#__PURE__*/ Math.max(i, /*#__PURE__*/ Math.min(w, /*#__PURE__*/ Math.round(S * u)));
